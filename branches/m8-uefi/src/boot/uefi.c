...
typedef struct {
    ...
    VOID*                       mps_table_floating_structure_addr;
    VOID*                       acpi_rsdp_addr;
} BootInfo;
...
)
{
    ...
    for (i = 0; i < efi_configuration_table_entry_count; i++) {
        ...
        if (compareEfiGUIDs( vendor_guid, MPS_TABLE_GUID ))
            boot_info->mps_table_floating_structure_addr = efi_config_table_entries[i].VendorTable;
        else if (compareEfiGUIDs( vendor_guid, EFI_ACPI_TABLE_GUID ))
            boot_info->acpi_rsdp_addr = efi_config_table_entries[i].VendorTable;
    }
