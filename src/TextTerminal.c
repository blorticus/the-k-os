...
static void TextTerminal_advanceCursorOneColumn( TextTerminal term ) {
    if (++term->cursorX >= term->glyphColumnsInFrameBuffer) {
        if (++term->cursorY >= term->glyphRowsInFrameBuffer) {
            TextTerminal_scrollOneLine( term );
...
static Error TextTerminal_PutRune( TextTerminal term, Rune rune ) {
    const uint8_t* glyphBitmap;

    switch (rune) {
        case (Rune)'\t':
            for (int i = 0; i < 4; i++)
                TextTerminal_PutRune( term, (Rune)' ' );
            break;
