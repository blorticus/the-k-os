...
// XXX: this relies on static allocation
static struct SimpleFontIterator_t g_SimpleFontIterator;

SimpleFontIterator SimpleFont_RuneRowIterator( SimpleFont font, char32_t rune ) {
    if (rune > font->numberOfEntriesInBitmap)
        return (SimpleFontIterator)0;

    g_SimpleFontIterator.font = font;
    g_SimpleFontIterator.theRuneOnWhichIamIterating = rune;
    g_SimpleFontIterator.nextRow = 0;
    g_SimpleFontIterator.currentByteOffsetInBitmap = (void*)((uint8_t*)(font->bitmap) + rune * font->NumberOfBytesPerRow * font->NumberOfRowsInARune);

    return &g_SimpleFontIterator;
}

void* SimpleFontIterator_RetrieveNextRow( SimpleFontIterator iterator ) {
    if (iterator->nextRow > iterator->font->NumberOfRowsInARune)
        return (void*)0;

    // XXX: make note of the fact that this return value may point to the actual underlying
    //      data storage, so it should not be manipulated by the caller
    void* bytesOfNextRow = iterator->currentByteOffsetInBitmap;

    iterator->currentByteOffsetInBitmap += iterator->font->NumberOfBytesPerRow;
    iterator->nextRow++;

    return bytesOfNextRow;
}

struct SimpleFont_t g_SimpleFont8x16 = {
    .PixelsPerRuneRow = 8,
    .NumberOfRowsInARune = 16,
    .NumberOfBytesPerRow = 1,
    .bitmap = (void*)FONT_MAP_8x16,
    .numberOfEntriesInBitmap = 256,
};

SimpleFont SimpleFont8x16 = &g_SimpleFont8x16;
